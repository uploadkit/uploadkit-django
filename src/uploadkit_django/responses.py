"""Map UploadKit exceptions to HTTP status / Django responses."""
...
def json_error_response(exc: UploaderError) -> JsonResponse:
    """Build a ``JsonResponse`` for an ``UploaderError``."""
    return JsonResponse(error_payload(exc), status=status_for_error(exc))
